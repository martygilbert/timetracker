 ...
 */
 ...
 class block_timetracker extends block_base {
    ...
    function get_content() {
        ...
        $clockin = optional_param('clockin', 0,PARAM_INT);
        $clockout = optional_param('clockout',0, PARAM_INT);
        $courseid = $COURSE->id;
        ...
        if($canmanage || $canview){
            ...
            $this->content->text .= $timeclockdataaction.' '.
                $OUTPUT->action_link($index, 'Earnings information').'<br />';
            ...
            $numtimeclock = $DB->count_records('block_timetracker_workerinfo',
                array('courseid'=>$courseid, 'timetrackermethod'=>0, 'deleted'=>0));
            ...
            if($numtimeclock > 0){
                ...
                if(!$pendingunits){ 
                    ...
                    $workers = $DB->get_records('block_timetracker_workerinfo',
                        array('deleted'=>0, 'courseid'=>$courseid));
                    $myparams['clockout']=1;
                    $myparams['ispending']=true;
                    $myparams['id'] = $courseid;
                    $clockouturl = new moodle_url($CFG->wwwroot.
                        '/blocks/timetracker/timeclock.php', $myparams);
                    $clockouticon = new pix_icon('clock_stop', 'Clock out',
                        'block_timetracker');
                    foreach ($pendingunits as $pending){
                        $clockouturl->params(
                            array('unitid'=>$pending->id, 'userid'=>$pending->userid));
                        $clockoutaction = $OUTPUT->action_icon($clockouturl, $clockouticon);
                        $this->content->text .='<li>'.$clockoutaction.' <a href="'.
            ...
            if (!$worker){// || ($worker->address=='0')){
                ...
                //check to see if there are alerts - Feature #271
                $hasalerts = has_course_alerts($courseid, $worker->id);
                if($hasalerts){
                    $alertsparams['id'] = $courseid;
                    $alertsparams['userid'] = $worker->id;

                    $alertsurl = new moodle_url($baseurl.'/managealerts.php', $alertsparams);
                    $alerticon= new pix_icon('alert','View Alerts', 'block_timetracker');
                    $alertaction= $OUTPUT->action_icon($alertsurl, $alerticon);

                    $this->content->text .= $alertaction.
                        '<span style="color: red">'.
                        $OUTPUT->action_link($alertsurl, ' View alerts ('.$hasalerts.')').
                        '</span><br />';
                ...
                if($worker->timetrackermethod == 0){
                    ...
                    if(!$pendingrecord){ 
                        ...
                        $timeclockicon = new pix_icon('clock_stop','Clock out', 
                            'block_timetracker');
                    ...
                    $this->content->text .= '<div style="text-align: left">';
                    $this->content->text .= 
                        $timeclockdataaction.' <a href="'.$index.
                        '">Earnings information</a><br />'.
                ...
                $this->content->text .= '<br /><span style="font-weight: bold;">'.
                    'Processed term earnings: '.
                    $OUTPUT->action_link($index,'$'. 
                    number_format(get_earnings_this_term($ttuserid, $COURSE->id, 0, true), 2)).
    ...
    */
    ...
    function instance_delete() {
        ...
        //deleting the course removes ALL associated worker data
