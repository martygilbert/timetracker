...
class timetracker_reports_form  extends moodleform {
    ...
    function definition() {
        ...
        if($this->userid == 0 && ($canmanage || $canview)){
            ...
            $workers =
                $DB->get_records('block_timetracker_workerinfo',
                    array('courseid'=>$this->courseid, 'deleted'=>0));
            if(!$workers){
        ...
        */
        ...
        $mform->addElement('html', get_popup_css());
        ...
        if(!$units){ //if they don't have them.
            ...
            foreach($units as $unit){
                $row='<tr>';
                if($this->userid == 0){
                    //error_log("before checking");
                    //don't show deleted workunits
                    if(!isset($workers[$unit->userid])) continue;
                ...
                if($canmanage || $canview){
                    ...
                    if ($unit->timesheetid && !$unit->submitted){
                        ...
                        $row .= '<td style="text-align: center">'.
                            html_writer::nonempty_tag('span',
                                html_writer::empty_tag('img', 
                                    array('src' => 
                                    $CFG->wwwroot.'/blocks/timetracker/pix/wait.png', 
                                    'class' => 'icon')).
                                '<span style="width: 250px;">'.
                                get_workunit_popupinfo($unit).  
                                '</span>',
                                array('class'=>'dropt')).
                            '</td>';
