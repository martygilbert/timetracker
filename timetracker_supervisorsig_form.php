...
class timetracker_supervisorsig_form extends moodleform {
   function timetracker_supervisorsig_form($courseid, $context){
    ...
    //Add all timesheets signed within the last 60 days
        $sql = 'SELECT * from '.$CFG->prefix.
            'block_timetracker_timesheet where courseid='.$this->courseid.
            ' AND workersignature > '.(time()-(60*86400)).
            ' AND supervisorsignature > 0 ORDER BY workersignature DESC'; 
        $timesheets = $DB->get_records_sql($sql);
        
        $mform->addElement('header','general','Timesheets from the last 60 days');

        if(!$timesheets){
            $mform->addElement('html','<br />No timesheets within the last 60 days.');
        } else {
            $mform->addElement('html','<br /><table align="center" border="1" cellspacing="10px"
                cellpadding="5px" width="85%">');
                $row = '<tr>';
                $row .= '
                    ...
                    <td style="font-weight: bold;">Signature</td>
                    <td style="font-weight: bold; text-align: right">Total hours</td>
                    <td style="font-weight: bold; text-align: right">Pay</td>
