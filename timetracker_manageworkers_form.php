...
class timetracker_manageworkers_form  extends moodleform {
    ...
    function definition() {
        ...
        if($canmanage){
            $reloadaction = $OUTPUT->action_icon($me, 
                new pix_icon('refresh', 'Refresh worker list',
                'block_timetracker'));
        ...
        if(!$workers = $DB->get_records('block_timetracker_workerinfo',
            array('courseid'=>$COURSE->id,'deleted'=>0),'lastname ASC, firstname ASC')){
            ...
            foreach ($workers as $worker){ 
                ...
                //Feature #133
                if($worker->timetrackermethod == 0) {//timeclock method
                    //check to see if they have a pending - if so, make it a clockout.
                    //if not, make it a clockin
                    $timeclockurl = new moodle_url($baseurl.'/timeclock.php', $urlparams);

                    $pendingrecord = $DB->get_record('block_timetracker_pending',
                        array('userid'=>$worker->id,'courseid'=>$COURSE->id));

                    if($pendingrecord){ //clock-out needed
                        $timeclockurl->params(array('clockout'=>1));
                        $timeclockicon = new pix_icon('clock_stop','Clock-out',
                            'block_timetracker');
                    } else {
                        $timeclockurl->params(array('clockin'=>1));
                ...
                $mform->setType('workerid['.$worker->id.']', PARAM_INT);
            }
